object Exercises {
  ...
  case class Shelter[+T <: Animal](animals: List[T]){
    def +[A >: T <: Animal](otherAnimals: A): Shelter[A] =
      Shelter(animals :+ otherAnimals)

    def ++[U >: T <: Animal](otherAnimals: Shelter[U]): Shelter[U] =
      Shelter(animals ++ otherAnimals.animals)
